...
def predict_video(video_path, video_buffer_folder=config.MAIN_PATH + "VideoBufferFolder" + os.sep, model_save_path=config.AUDIO_FACE_COMBINED_MODEL_SAVE_PATH):
    ...
    # Check if model file is a Git LFS pointer (not the actual model)
    if os.path.exists(model_save_path):
        file_size = os.path.getsize(model_save_path)
        # Git LFS pointer files are typically very small (< 1KB)
        if file_size < 1024:
            with open(model_save_path, 'r') as f:
                first_line = f.readline().strip()
                if first_line == "version https://git-lfs.github.com/spec/v1":
                    raise Exception(
                        f"ERROR: The combined model file is a Git LFS pointer, not the actual model.\n"
                        f"The model file should be ~137MB but is only {file_size} bytes.\n\n"
                        f"To fix this, you need to install Git LFS and download the actual model:\n"
                        f"1. Install Git LFS: brew install git-lfs (on macOS) or visit https://git-lfs.github.com/\n"
                        f"2. Run: git lfs install\n"
                        f"3. Run: git lfs pull models/audio_face_combined/audio_face_combined_model.pth\n\n"
                        f"Alternatively, download the model file directly from the GitHub repository."
                    )
    ...
    try:
        combined_model = torch.load(model_save_path, weights_only=False, map_location=config.device)
        combined_model.to(config.device).eval()
    except Exception as e:
        if "invalid load key" in str(e).lower() or "unpicklingerror" in str(type(e).__name__).lower():
            raise Exception(
                f"ERROR: Failed to load the combined model file. The file may be corrupted or a Git LFS pointer.\n"
                f"File path: {model_save_path}\n"
                f"File size: {os.path.getsize(model_save_path) if os.path.exists(model_save_path) else 'N/A'} bytes\n\n"
                f"If the file is very small (< 1KB), it's likely a Git LFS pointer. Install Git LFS:\n"
                f"1. Install: brew install git-lfs (macOS) or visit https://git-lfs.github.com/\n"
                f"2. Run: git lfs install && git lfs pull models/audio_face_combined/audio_face_combined_model.pth\n\n"
    ...
    # Check if any valid faces were detected
    if frames_face_images_X is None or frames_face_images_X.size == 0 or len(frames_face_images_X.shape) < 2:
        raise Exception(
            "ERROR: No faces detected in the video.\n\n"
            "Possible reasons:\n"
            "1. The video doesn't contain any visible faces\n"
            "2. The faces in the video are too small, blurry, or obscured\n"
            "3. The video format is not supported\n"
            "4. The video is too short (minimum 1.5 seconds required)\n\n"
            "Please try a different video with clearly visible faces."
        )
    ...
    if len(frames_face_images_X.shape) < 2 or frames_face_images_X.shape[1] != 3:
