...
def split_video_into_equal_parts(full_video="full.mp4", save_dir="", divide_into_secs=combined_config.VIDEO_ANALYSE_WINDOW_SECS, min_vid_len=config.MIN_VID_LEN):
    ...
    if not os.path.exists(full_video):
        raise Exception(f"Video file not found: {full_video}")
    ...
    try:
        ...
        print(f"Error loading video with MoviePy: {e}")
        # Fallback to OpenCV for duration
        import cv2
        cap = cv2.VideoCapture(full_video)
        if not cap.isOpened():
            cap.release()
            raise Exception(f"Could not open video file: {full_video}. File may be corrupted or in unsupported format.")
        ...
        if fps > 0 and frame_count > 0:
            ...
            raise Exception(f"Could not determine video duration. Video file may be corrupted or in unsupported format. FPS: {fps}, Frame count: {frame_count}")
    
    if current_duration is None or current_duration <= 0:
        raise Exception(f"Invalid video duration: {current_duration}. Video file may be corrupted.")
    ...
    while current_duration >= min_vid_len:
        ...
        try:
            ...
            print(f"Error processing video segment {start}-{end}: {e}")
