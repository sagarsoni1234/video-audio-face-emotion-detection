...
def _get_prediction(
    ...
    try:
        ...
        result = face_mesh.get_mesh(image=cv2.cvtColor(img, cv2.COLOR_RGB2BGR), upscale_landmarks=True, showImg=False, print_flag=verbose, return_mesh=True)
    except Exception as e:
        raise Exception(f"Face mesh failed: {str(e)}")
    if result is None:
        if verbose:
